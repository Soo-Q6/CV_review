...
for i = 1 : 2
   ...
   xlo_i = max(floor(min(bounds{i,1}(1,:))),1);
   xhi_i = min(ceil(max(bounds{i,1}(1,:))),size(imTotal,2));
   ylo_i = max(floor(min(bounds{i,1}(2,:))),1);
   yhi_i = min(ceil(max(bounds{i,1}(2,:))),size(imTotal,1));
