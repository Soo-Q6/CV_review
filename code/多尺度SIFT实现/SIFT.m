...
if interactive >= 2
   ...
   imshow(uint8(im));%showIm(im);
   hold on;
   quiver(pos(:,1),pos(:,2),cos(orient),sin(orient));%display_keypoints( pos, scale(:,3), orient, 'y' );
   %resizeImageFig( fig, size(im), 2 );
